...
def test_writing_a_proof_without_a_salt_is_refused(run, project):
    """An unsalted digest of a low-entropy identifier is reversible.

    The proof is the artifact handed to people who must not learn who the subject
    was, so producing one without a secret salt has to fail loudly rather than
    quietly emit something that identifies them.
    """
    run("ingest")
    result = run(
        "erase",
        ...
        "raw.events",
        "--proof",
        str(project / "unsalted.json"),
    )
    assert result.exit_code != 0
    assert "secret salt" in result.output
    assert not (project / "unsalted.json").exists()
...
def test_plan_emits_json_for_a_pipeline(run):
    """CI gating is the headline use, and screen-scraping prose is how an
    orchestrator ends up rebuilding the wrong thing after a wording change."""
    run("ingest")
    result = run("plan", "--dirty", "raw.events@dt=2026-03-14", "--json")
    assert result.exit_code == 0

    payload = json.loads(result.output)
    assert payload["order"], "an orchestrator needs the build order"
    entry = payload["datasets"]["duckdb/silver.events"]
    assert entry["rendered"] == ["dt=2026-03-14T00:00:00/region=ANY"]
    assert entry["widened"] is False
    # Partition values keep their type, so a consumer can compare them.
    assert entry["partitions"][0][0][1]["t"] == "datetime"


def test_an_empty_plan_is_still_valid_json(run):
    """A pipeline branching on the output must not have to special-case "nothing"."""
    run("ingest")
    result = run("plan", "--dirty", "raw.events@dt=1970-01-01", "--json")
    assert json.loads(result.output)["datasets"] is not None
