...
from fathom.adapters.engines.bigquery import BigQueryAdapter, grain_of, parse_partition_id
from fathom.adapters.engines.databricks import DatabricksAdapter
from fathom.adapters.engines.snowflake import SnowflakeAdapter
...
def test_identifiers_are_escaped_not_just_values():
    """Values were escaped and identifiers were not.

    A partition field or `--key-column` containing the quote character broke out of
    its own quoting, and the statements these names land in include DELETE.
    """
    from fathom.adapters.predicates import identifier, render_predicate
    from fathom.core.types import KeyPredicate, PartitionField, PartitionSpec

    assert identifier('reg"ion') == '"reg""ion"'

    spec = PartitionSpec.of(PartitionField.value('reg"ion'))
    rendered = render_predicate(spec, [KeyPredicate.of(**{'reg"ion': "eu"})])
    assert rendered == '("reg""ion" = \'eu\')'


# -- configuration and rendering across all three warehouses -------------------


def _table(system: str) -> object:
    return normalize_table("db.sch.events", system=system)


def test_every_engine_declares_partitioning_and_registers_a_model():
    """`declare` and `register_model` are how a project tells an adapter what it owns."""
    for adapter, dataset in (
        (SnowflakeAdapter(runner=RecordedRunner({}, strict=False)), _table("snowflake")),
        (DatabricksAdapter(runner=RecordedRunner({}, strict=False)), _table("databricks")),
        (BigQueryAdapter(runner=RecordedRunner({}, strict=False)), _table("bigquery")),
    ):
        adapter.declare(dataset, DAY)
        assert adapter.describe_partitioning(dataset) == DAY

        adapter.register_model(dataset, "SELECT 1")
        statements = adapter.render_rebuild(dataset, [KeyPredicate.of(dt=datetime(2026, 3, 14))])
        assert statements, f"{adapter.name} rendered no rebuild"
        assert any("DELETE" in s.upper() for s in statements)


def test_databricks_declare_records_a_location():
    adapter = DatabricksAdapter(runner=RecordedRunner({}, strict=False))
    dataset = _table("databricks")
    adapter.declare(dataset, DAY, location="s3://lake/events")
    assert adapter.describe_partitioning(dataset) == DAY
    assert adapter.location_of(dataset) == "s3://lake/events"


def test_erase_renders_a_subject_scoped_delete():
    """The statement must name the subject *and* the partitions, never one alone."""
    for adapter, dataset in (
        (SnowflakeAdapter(runner=RecordedRunner({}, strict=False)), _table("snowflake")),
        (DatabricksAdapter(runner=RecordedRunner({}, strict=False)), _table("databricks")),
        (BigQueryAdapter(runner=RecordedRunner({}, strict=False)), _table("bigquery")),
        ...
        adapter.erase(
            ...
            key_column="user_id",
            subject="u1",
            partitions=[KeyPredicate.of(dt=datetime(2026, 3, 14))],
        )
        issued = " ".join(adapter.runner.executed)
        assert "DELETE" in issued.upper()
        assert "u1" in issued
        assert "user_id" in issued


def test_fetch_queries_returns_nothing_when_the_log_is_empty():
    for adapter in (
        SnowflakeAdapter(runner=RecordedRunner({}, strict=False)),
        DatabricksAdapter(runner=RecordedRunner({}, strict=False)),
        BigQueryAdapter(runner=RecordedRunner({}, strict=False)),
    ):
        assert list(adapter.fetch_queries(None)) == []


def test_databricks_lineage_token_holds_back_by_the_system_table_lag():
    from fathom.adapters.base import LineageEvent
    from fathom.adapters.engines.databricks import SYSTEM_TABLE_LAG

    adapter = DatabricksAdapter(runner=RecordedRunner({}, strict=False))
    latest = datetime(2026, 3, 14, 12, 0, tzinfo=UTC)
    events = [
        LineageEvent(
            src=_table("databricks"),
            dst=normalize_table("db.sch.gold", system="databricks"),
            observed=latest,
        )
    ]
    token = adapter.lineage_token(events)
    assert token, "an adapter that reports lineage must return a resume token"
    # Advancing to the newest event would permanently skip rows still in flight.
    assert token < latest.isoformat()
    assert (latest - SYSTEM_TABLE_LAG).isoformat() == token
