...
from ..core.ids import normalize_table
from ..core.partitions import UNBOUNDED, FieldMapping, PartitionMapping, Passthrough, TimeWindow
...
def _is_cte_reference(table: exp.Table, cte_names: set[str]) -> bool:
    """True when this table node is really a reference to a CTE declared in the same
    statement. A qualified name can never be one, so `raw.recent` stays a real table
    even when a CTE is also called `recent`.
    """
...
def _projection_owner(projection: Any, alias_by_table: Mapping[str, DatasetId]) -> DatasetId | None:
    """Which source dataset the single column behind this projection comes from.

    None when the projection reads no column, reads several, or carries no qualifier
    we can resolve — all of which mean the caller must not assume an owner.
    """
    ...
    if not qualifier:
        return None
    return alias_by_table.get(qualifier)
...
def extract(
    ...
    *,
    ...
    for statement in statements:
        ...
        # A CTE name parses as a table but is not one. Left in, every `WITH recent AS
        # (...)` mints a phantom dataset that holds edges the real table should have.
        cte_names = {
        ...
        source_tables = [
            ...
            if t is not target_table
            and not isinstance(t.parent, exp.Drop)
            and not _is_cte_reference(t, cte_names)
        ]
        ...
        for ds in sources:
            ...
            if isinstance(statement, exp.Merge) or not dst_spec.fields:
                ...
                fields: list[tuple[str, FieldMapping]] = []
                for f in dst_spec.fields:
                    projection = projections.get(f.name)
                    owner = _projection_owner(projection, alias_by_table)
                    # `SELECT a.dt ... FROM a JOIN b` says how a's partitions reach the
                    # target and nothing at all about b's. Attributing the window to
                    # every source lets a change in b map to the wrong partition of the
                    # target, which under-invalidates. Only the owner gets a bound; an
                    # unresolvable owner among several candidates gets none.
                    if (owner is not None and owner != ds) or (owner is None and ambiguous):
                        fields.append((f.name, UNBOUNDED))
                    else:
                        fields.append(
                            (f.name, _infer_field(projection, src_spec, f.name, dst_spec))
                        )
                extraction.mappings[ds] = PartitionMapping(fields=tuple(fields))
