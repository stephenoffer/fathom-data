...
# How each engine spells "this partition's value is NULL" in a directory name.
_NULL_PARTITION = frozenset({"__HIVE_DEFAULT_PARTITION__", "__NULL__", ""})
...
def parse_hive_partitions(path: str) -> dict[str, str]:
    ...
    Values are percent-decoded, because Spark and Hive escape anything that cannot
    sit in a path segment: a region written as ``eu west`` lands on disk as
    ``region=eu%20west``, and comparing the raw form against a partition value read
    from a catalog never matches.
    """
    out: dict[str, str] = {}
...
class PathTemplate:
    ...
    def _pattern(self) -> re.Pattern[str]:
        """Compiled once. A storage listing runs this against every object, and
        recompiling per path made template extraction the dominant cost of profiling
        a large prefix."""
        parts: list[str] = []
    ...
    def extract(self, path: str) -> dict[str, str]:
        """Bindings this template finds in a path, percent-decoded."""
        m = self._pattern.search(path)
        return {k: unquote(v) for k, v in m.groupdict().items() if v is not None} if m else {}
...
def key_from_path(
    ...
    *,
    ...
    for f in spec.fields:
        if f.kind == "value":
            found: object = raw.get(f.name, ANY)
            if found in _NULL_PARTITION:
                # Hive and Spark spell a null partition with a sentinel directory.
                # Binding the literal string means the key never matches the null
                # partition the catalog reports, so its rows are never invalidated.
                found = None
            bindings.append((f.name, found))
            continue
