...
from ..core.partitions import Passthrough, TimeWindow
...
class GraphStats:
    ...
    def summary(self) -> str:
        """The measurements as one printable line."""
        parts = [
...
def diameter(graph: Graph) -> int:
    ...
    """
    return max(levels(graph), default=0)
...
def longest_chain(graph: Graph) -> list[DatasetId]:
    ...
    Walked back from the deepest dataset through the parent that put it there, which
    is O(V+E). Enumerating every root-to-leaf path instead is exponential, and the
    cap that made it terminate also made it wrong: on a graph with more paths than
    the cap it returned some chain, not the longest one.
    """
    from .query import _depths, topological_order

    depth = _depths(graph)
    if not depth:
        return []

    ranked = {ds: i for i, ds in enumerate(topological_order(graph))}
    # Deepest wins; ties break on name so the answer is stable and diffable.
    deepest = min(depth, key=lambda ds: (-depth[ds], str(ds)))

    chain = [deepest]
    node = deepest
    while depth[node] > 0:
        # The parent one level up, on the forward direction of the order, is the one
        # this dataset's depth came from.
        parent = next(
            (
                e.src
                for e in sorted(graph.in_edges(node), key=lambda e: str(e.src))
                if e.src != node
                and depth.get(e.src, -1) == depth[node] - 1
                and ranked.get(e.src, -1) < ranked.get(node, -1)
            ),
            None,
        )
        if parent is None:  # only reachable through a cycle
            break
        chain.append(parent)
        node = parent
...
def _reachability_counts(graph: Graph, *, downstream: bool) -> dict[DatasetId, int]:
    """How many datasets each one reaches, in one pass over the whole graph.

    Running a traversal per dataset is O(V x (V+E)); on a warehouse-sized graph that
    is minutes. Over a DAG the answer is a dynamic program along a topological order,
    where each node's reachable set is the union of its children's — held as an
    integer bitset, so the union is one `|` and the count is one `bit_count`.

    A cyclic graph falls back to the per-node walk. Reachability inside a cycle is
    not a fold over an order, and a wrong-but-fast answer is worth nothing here.
    """
    if graph._on_cycle():
        walk = descendants if downstream else ancestors
        return {ds: len(walk(graph, ds)) for ds in graph.datasets}

    order = topological_order(graph)
    bit = {ds: 1 << index for index, ds in enumerate(order)}
    reach: dict[DatasetId, int] = {}
    for ds in reversed(order) if downstream else order:
        acc = 0
        edges = graph.out_edges(ds) if downstream else graph.in_edges(ds)
        for edge in edges:
            other = edge.dst if downstream else edge.src
            if other == ds:
                continue
            acc |= reach.get(other, 0) | bit[other]
        reach[ds] = acc
    return {ds: reach.get(ds, 0).bit_count() for ds in graph.datasets}
...
def reach_score(graph: Graph) -> dict[DatasetId, int]:
    ...
    """
    return _reachability_counts(graph, downstream=True)
...
def bottlenecks(graph: Graph, *, limit: int = 10) -> list[tuple[DatasetId, int]]:
    ...
    """
    up_counts = _reachability_counts(graph, downstream=False)
    down_counts = _reachability_counts(graph, downstream=True)
    scored: list[tuple[DatasetId, int]] = []
    for ds in graph.datasets:
        up, down = up_counts.get(ds, 0), down_counts.get(ds, 0)
        if up and down:
...
def health_report(graph: Graph) -> HealthReport:
    ...
    # Computed once. This walks the whole graph, and it was being run twice here on
    # top of the call inside `graph_stats` below.
