...
from ..core.types import DatasetId, KeyPredicate, covered_by
from ..graph.model import InvalidationPlan
...
class Fingerprinter(Protocol):
    ...
    def fingerprints(self, dataset: DatasetId) -> dict[KeyPredicate, str]:
        """Content hash per partition, so a rebuild's effect can be compared."""
        ...

    def full_rebuild(self, dataset: DatasetId) -> str:
        """Rebuild the whole dataset. This writes, at full warehouse cost."""
        ...


@runtime_checkable
class ShadowLedger(Protocol):
    """Where shadow results are accumulated.

    One method wide on purpose. Shadow mode needs somewhere to put a number; it does
    not need the store, and depending on the store would make the persistence layer
    and the thing it measures import each other.
    """

    def record_shadow(self, observation: ShadowObservation) -> None:
        """Persist one graded comparison."""
        ...
...
class ShadowObservation:
    ...
    def savings(self) -> float:
        """Fraction of partitions the plan avoided rebuilding."""
...
class ShadowReport:
    ...
    def missed_total(self) -> int:
        """Partitions wrongly called clean, across every dataset compared."""
    ...
    def summary(self) -> str:
        """The report as text, headline verdict first."""
        if not self.results:
    ...
    def persist(self, store: ShadowLedger) -> None:
        """Write every result to a store as `ShadowObservation` rows."""
...
def run(
    ...
    **This writes.** It is the one entry point in the library that does. Grading a
    plan means knowing what a full rebuild would actually have produced, and the only
    way to know that is to run one — so every dataset passed here is rebuilt through
    `engine.full_rebuild`, at full warehouse cost, against whatever the engine is
    pointed at. Everything else in `fathom` is read-only by default; this is not, and
    it is worth pointing at a staging copy the first time.
    """
    report = ShadowReport()
