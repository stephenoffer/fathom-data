...
from ..core.errors import StorageAccessError, hint_for
...
def is_remote(uri: str) -> bool:
    """True when this URI addresses object storage rather than local disk.

    Decides whether an operation is worth batching: a thousand local `stat` calls
    are free, and a thousand remote ones are a minute of latency.
    """
...
class FileInfo:
    ...
    @property
    def suffix(self) -> str:
        """Lower-cased file extension including the dot, or empty for none.

        Used to keep `_SUCCESS` markers and checkpoint files out of a profile that
        is meant to describe data.
        """
...
class FileSystem(Protocol):
    ...
    def exists(self, uri: str) -> bool:
        """Whether anything is present at this URI."""
        ...

    def is_dir(self, uri: str) -> bool:
        """Whether this URI behaves as a directory.

        Object stores have no directories, so an implementation should report True
        for a prefix that has anything underneath it.
        """
        ...

    def ls(self, uri: str, *, recursive: bool = True) -> Iterator[FileInfo]:
        """Objects under a prefix. Directory entries are not yielded."""
        ...

    def info(self, uri: str) -> FileInfo:
        """Size, etag, and modification time for one object."""
        ...

    def read_bytes(self, uri: str) -> bytes:
        """Whole object as bytes. For footers, prefer `open` and seek."""
        ...

    def read_text(self, uri: str) -> str:
        """Whole object decoded as UTF-8."""
        ...

    def open(self, uri: str) -> BinaryIO:
        """A seekable binary handle, so a Parquet footer can be read alone."""
        ...

    def delete(self, uri: str) -> None:
        """Remove one object. Never recursive: erasure names its targets."""
        ...
...
class FsspecFileSystem:
    ...
    def is_dir(self, uri: str) -> bool:
        """Whether this URI behaves as a directory, prefixes included."""
    ...
    def ls(self, uri: str, *, recursive: bool = True) -> Iterator[FileInfo]:
        """Objects under a prefix, directory entries filtered out.

        A missing prefix yields nothing rather than raising: absent and empty are
        the same answer to "what is here", unlike a permission failure, which
        `_wrapped` turns into `StorageAccessError`.
        """
        target = self._strip(uri)
        with _wrapped(uri):
    ...
    def open(self, uri: str) -> BinaryIO:
        """A seekable binary handle, so a Parquet footer can be read without the body."""
        with _wrapped(uri):
