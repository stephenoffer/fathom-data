...
from ...core.grains import Grain, truncate
...
class IcebergCatalog:
    ...
    def declare(self, dataset: DatasetId, spec: PartitionSpec) -> None:
        """Record a partition spec for a table the catalog cannot describe."""
    ...
    def is_iceberg_table(self, dataset: DatasetId) -> bool:
        """True when Iceberg metadata sits under this prefix."""
        try:
    ...
    def describe_partitioning(self, dataset: DatasetId) -> PartitionSpec:
        """The table's partition spec, translated from Iceberg's own."""
    ...
    def changed(self, dataset: DatasetId, since: Token | None) -> ChangeSet:
        """Partitions touched since a snapshot id, from Iceberg's manifests."""
        table = self._table(dataset)
