...
class SnowflakeAdapter:
    ...
    def describe_partitioning(self, dataset: DatasetId) -> PartitionSpec:
        """The declared spec: Snowflake exposes no partitioning to read."""
            ...
            for modified in _as_list(_row(row, "objects_modified")):
                ...
                for src, columns in by_source.items():
                    yield LineageEvent(
                        ...
                        # Names the source of knowledge, not the execution. The
                        # query id changes every run, and evidence is an edge's
                        # identity in the store, so folding it in makes one
                        # dependency accumulate a row per query forever.
                        evidence="snowflake:access_history",
