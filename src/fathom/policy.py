    ...
    *,
    ...
    for _round in range(max_rounds):
        ...
        for edge in graph.edges:
            source_labels = {
                ref: values for ref, values in labels.items() if ref.dataset == edge.src
            }
            if not source_labels:
                continue
            if edge.columns:
                for src_col, dst_col in edge.columns:
                    for label in source_labels.get(ColumnRef(edge.src, src_col), set()):
                        moved = label.damped(damping, f"propagated:{edge.src}")
                        changed |= _merge(labels, ColumnRef(edge.dst, dst_col), moved)
            else:
                # No column-level lineage on this edge: we know the label reached the
                # target dataset but not which column carries it.
                for values in source_labels.values():
                    for label in values:
                        moved = label.damped(damping, f"propagated-unattributed:{edge.src}")
                        changed |= _merge(labels, ColumnRef(edge.dst, UNATTRIBUTED), moved)
        if not changed:
            break
...
def enforce(
    ...
    *,
    ...
    """
    by_dataset: Mapping[DatasetId, SinkPolicy] = {p.dataset: p for p in policies}
    report = PolicyReport()
    ...
    for ref, values in sorted(labels.items(), key=lambda kv: (str(kv[0].dataset), kv[0].column)):
        ...
        for label in sorted(values):
            if label.name not in policy.forbid:
                continue
            if not label.confirmed and label.confidence < min_confidence:
                continue
