...
from .types import ColumnRef, DatasetId
...
def matches(term: Term, ds: DatasetId, *, labels: LabelSet | None = None) -> bool:
    ...
    if term.kind == "tag":
        if not labels:
            return False
        return any(
            ref.dataset == ds and any(label.name == term.body for label in values)
...
def _resolve_term(graph: Graph, term: Term, *, labels: LabelSet | None = None) -> set[DatasetId]:
...
def select_columns(
    graph: Graph, selector: str, *, labels: LabelSet | None = None
) -> list[ColumnRef]:
...
def explain(graph: Graph, selector: str, *, labels: LabelSet | None = None) -> str:
...
def label_index(labels: LabelSet) -> Mapping[DatasetId, set[str]]:
    """Dataset to label names, the shape `tag:` matching wants."""
